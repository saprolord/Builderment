...
function factorycalc(reverse) {
  ...
  const emagnet = new MatDef(
    "Electromagnet",
    [3, 7.5],
    [iron_ingot, copper_wire],
    [2, 6],
    22
  );
  const metal_frame = new MatDef(
    "Metal Frame",
    [3, 5],
    [wood_frame, iron_plate],
    [1, 4],
    23
  );
  const steel = new MatDef("Steel", [5, 7.5], [graphite, iron], [1, 6], 24);
  const steel_rod = new MatDef("Steel Rod", [1, 15], [steel], [3], 25);
  const rotor = new MatDef(
    "Rotor",
    [3, 10],
    [steel_rod, iron_plate],
    [1, 2],
    26
  );
  const concrete = new MatDef(
    "Concrete",
    [5, 7.5],
    [sand, steel_rod],
    [10, 1],
  ...
  const motor = new MatDef(
    "Electric Motor",
    [4, 3],
    [battery, iron_gear, rotor],
    [1, 4, 2],
    29
  );
  const circuit = new MatDef(
    "Logic Circuit",
    [3, 10],
    [copper_wire, silicon],
    [3, 2],
  ...
  const computer = new MatDef(
    "Computer",
    [4, 7.5],
    [heat_sink, metal_frame, circuit],
    [3, 1, 3],
    33
  );
  const ind_frame = new MatDef(
    "Industrial Frame",
    [4, 3],
    [concrete, metal_frame, carbide],
    [6, 2, 8],
    34
  );
  const gyroscope = new MatDef(
    "Gyroscope",
    [3, 5],
    [copper_wire, rotor],
    [12, 2],
  ...
  const stabilizer = new MatDef(
    "Stabilizer",
    [4, 2.5],
    [computer, motor, gyroscope],
    [1, 1, 2],
    36
  );
  const mag_field = new MatDef(
    "Mag. Field Generator",
    [6, 1.5],
    [stabilizer, ind_frame, emagnet, nanowire],
    [1, 1, 10, 10],
    37
  );
  const quantum = new MatDef(
    "Quantum Entangler",
    [3, 1],
    [mag_field, stabilizer],
    [1, 2],
    38
  );
  const microscope = new MatDef(
    "Electron Microscope",
    [6, 2.5],
    [nanowire, emagnet, lens, metal_frame],
    [2, 8, 4, 2],
    39
  );
  const turbocharg = new MatDef(
    "Turbocharger",
    [6, 4],
    [iron_gear, circuit, nanowire, coupler],
    [8, 4, 2, 4],
    40
  );
  const supercomp = new MatDef(
    "Super Computer",
    [6, 2],
    [computer, heat_sink, turbocharg, coupler],
    [2, 8, 1, 8],
    41
  );
  const atomic = new MatDef(
    "Atomic Locator",
    [6, 2],
    [supercomp, microscope, concrete, copper_wire],
    [2, 2, 24, 50],
    42
  );
  const energy_cube = new MatDef(
    "Energy Cube",
    [3, 2],
    [battery, ind_frame],
    [2, 1],
  ...
  const tank = new MatDef(
    "Tank",
    [4, 6],
    [glass, concrete, carbide],
    [2, 4, 4],
    44
  );
  const compressor = new MatDef(
    "Matter Compressor",
    [6, 2],
    [ind_frame, turbocharg, motor, tank],
    [1, 2, 2, 1],
    45
  );
  const particle = new MatDef(
    "Particle Glue",
    [1, 20],
    [compressor],
    [0.1],
  ...
  const duplicator = new MatDef(
    "Matter Duplicator",
    [6, 2 / 3],
    [atomic, quantum, energy_cube, particle],
    [4, 2, 5, 100],
    47
  );
  const earth_token = new MatDef(
    "Earth Token",
    [7, 60 / 42],
    [duplicator],
    [1],
  ...
  const nuclear_fuel = new MatDef(
    "Nuclear Fuel Cell",
    [4, 2],
    [empty_fuel, steel_rod, enriched_ur],
    [1, 1, 1],
    52
  );
  ...
  for (i = 0; i < materialdata.length; i++) {
    materialdata[i].factory[1] =
      materialdata[i].factory[1] *
      boostFactory[levelFactory[materialdata[i].factory[0]]];
  }
  ...
  if (reverse == true) {
    var extractorQty = [
      document.getElementById("wood_extractor_qty").value,
    ...
    for (var i = 0; i < extractor_ratio.length; i++) {
      if (materialtotal[i] == 0) {
        ...
        extractor_ratio[i] =
          (extractorQty[i] * materialdata[i].factory[1]) / materialtotal[i];
      }
    }
    ...
    document.getElementById("limit_resource").innerHTML =
...
function reversefactorycalc() {
  ...
  var extractor_ratio = [
    extractorQty[0] / materialtotal[0],
  ...
  alert(
    "You are limited by the amount of " && materialdata[j].name && " extractors"
  );
...
function calculate2(material, rate, divParentId, boxId) {
...
function materialOutput() {
  ...
  for (var i = 0; i < materialtotal.length; i++) {
    ...
    if (i < 10) {
      matId = "mat0" + i;
      document.getElementById(matId).innerHTML =
...
function createMatBox(material, rate, factory, qty, divParentId, boxId, matId) {
  ...
  matBox.id = boxId;
  sign.innerHTML =
    '<img src="https://raw.githubusercontent.com/saprolord/saprolord.github.io/main/image/Minus_Sign.png" style="width:15px;height:15px;" alt="-">';
...
function hideChild(childId) {
  ...
  document.getElementById("matBox" + childId).lastElementChild.innerHTML =
    innerHTML =
      '<img src="https://raw.githubusercontent.com/saprolord/saprolord.github.io/main/image/Plus_Sign.png" style="width:15px;height:15px;" alt="-">';
}
